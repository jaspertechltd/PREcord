...
## 2024-05-25 - In-memory cache for frequently accessed metadata in Jetpack Compose
...
**Action:** Always use an in-memory cache (like ConcurrentHashMap) for frequently accessed metadata. Always cache negative/empty states (like 'file not found') to avoid persistent I/O blocking. Return deep copies of objects containing collections (using e.g., .toMutableList() or .toList()) to prevent implicit cache mutation and missed recompositions.
