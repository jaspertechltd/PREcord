...
## 2025-02-12 - Prevent N+1 I/O blocking during Jetpack Compose LazyColumn rendering
...
**Action:** Always use an in-memory cache (like `ConcurrentHashMap`) for frequently accessed metadata to avoid persistent I/O blocking. Ensure to return deep copies of objects containing collections (using e.g., `.toMutableList()` or `.toList()`) to prevent implicit cache mutation and missed recompositions.
